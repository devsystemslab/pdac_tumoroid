...
def compute_gcs(
    ...
    if latent_knng_connectivities_key in adata.obsp:
        ...
        for i, batch in enumerate(unique_batches):
            print(f"Computing latent nearest neighbor graph for {batch_key} {batch}...")
            compute_knn_graph_connectivities_and_distances(
                adata=adata_batch_list[i],
                feature_key=latent_key,
                knng_key=latent_knng_key,
    ...
    if batch_key is None:
        ...
        for i, batch in enumerate(unique_batches):
            ...
            n_neighbors = adata_batch_list[i].uns[f"{latent_knng_key}_n_neighbors"]
            ...
            batch_gcd = sp.linalg.norm(batch_connectivities_diff, ord="fro")
            batch_gcs = 1 - (
                batch_gcd**2 / (n_neighbors * 2 * batch_connectivities_diff.shape[0])
            )
