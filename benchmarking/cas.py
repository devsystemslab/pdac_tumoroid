...
def compute_cas(
        ...
        for i in range(len(adata_batch_list)):
            if i == 0:  # first batch
                ...
                batch_connectivities.append(
                    sp.hstack(
                        (
                            adata_batch_list[0].obsp[spatial_knng_connectivities_key],
    ...
    adata.uns[f"{cell_type_key}_spatial_nhood_enrichment"]["zscore"] = adata.uns[
        f"{cell_type_key}_nhood_enrichment"
    ]["zscore"]
    ...
    nhood_enrichment_zscores_diff = nhood_enrichment_zscores_diff[
        ~np.isnan(nhood_enrichment_zscores_diff)
    ]
    ...
    # function
    cad_norm = math.atan(cad / nhood_enrichment_zscores_diff.shape[0]) / (math.pi / 2)
    cas = 1 - cad_norm
    return cas
