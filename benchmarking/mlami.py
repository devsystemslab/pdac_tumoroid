...
from benchmarking.utils import compute_knn_graph_connectivities_and_distances
...
def compute_mlami(
    ...
    if spatial_knng_connectivities_key in adata.obsp:
        ...
        for i, batch in enumerate(unique_batches):
            print(
                f"Computing spatial nearest neighbor graph for {batch_key} {batch}..."
            )
            compute_knn_graph_connectivities_and_distances(
    ...
    clustering_resolutions = np.linspace(
        start=min_res, stop=max_res, num=res_num, dtype=np.float32
    )
    ...
    if batch_key is None:
        ...
        for i, batch in enumerate(unique_batches):
            print(f"Computing spatial Leiden clusterings for {batch_key} {batch}...")
            # Calculate spatial Leiden clustering for different resolutions
            for resolution in clustering_resolutions:
                sc.tl.leiden(
                    adata=adata_batch_list[i],
    ...
    for resolution in clustering_resolutions:
        ...
        if batch_key is not None:
            for i, batch in enumerate(unique_batches):
                adata_batch_list[i].obs[f"leiden_latent_{str(resolution)}"] = adata.obs[
                    f"leiden_latent_{str(resolution)}"
                ]
...
def _compute_ami(
    adata: AnnData, cluster_group1_key: str, cluster_group2_key: str
) -> float:
    """
    ...
    ami = adjusted_mutual_info_score(
        cluster_group1, cluster_group2, average_method="arithmetic"
    )
    return ami
