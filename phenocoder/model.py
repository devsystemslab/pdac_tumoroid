...
class CVAE(Model):
    def __init__(self,
        ...
        self.reconstruction_loss_tracker_val = keras.metrics.Mean(name="reconstruction_loss_val")
    ...
    def build_encoder(self):
        ...
        for i, n in enumerate(self.conv_layers):
            if i == 0:
                x = layers.Conv2D(n, 3, activation="relu", strides=2, padding="same", name=f'conv_{n}')(encoder_inputs)
            else:
                x = layers.Conv2D(n, 3, activation="relu", strides=2, padding="same", name=f'conv_{n}')(x)
        x = layers.Flatten()(x)
    ...
    def build_decoder(self):
        ...
        x = layers.Dense(dim_0 * dim_1 * self.conv_layers[0], activation="relu")(latent_inputs)
        if self.dropout is not None:
            x = layers.Dropout(self.dropout)(x)
        x = layers.Reshape((dim_0, dim_1, self.conv_layers[0]))(x)

        for n in self.conv_layers[::-1]:
            x = layers.Conv2DTranspose(n, 3, activation="relu", strides=2, padding="same", name=f'conv_{n}')(x)

        decoder_outputs = layers.Conv2DTranspose(self.input_shape[-1], 3, activation="sigmoid", padding="same")(x)
...
class CondCVAE(CVAE):
    ...
    def test_step(self, data):
        ...
        return {"loss": self.total_loss_tracker_val.result(),
                "reconstruction_loss": self.reconstruction_loss_tracker_val.result(),
                "kl_loss": self.kl_loss_tracker_val.result()}
