...
def get_count_cycle(well: str, plate: str, cycle: str, dir_screen):
    """
    Get count for cycle
    :param well:
    :param plate:
    ...
    """
    dir_features = Path(
        dir_screen, plate, f"{plate}-{cycle}", "features", "nuclei", "TIF_OVR_BG"
    )
...
def detect_2d_plane(adata, plate):
    ...
    results = pd.DataFrame(results).assign(plate=plate)
    results = (
        results.groupby(["well", "plate"]).agg({"plane_detected": "any"}).reset_index()
    )
    return results
...
def qc_empty_wells(
    ...
    df_qc["count_cycle_1"] = df_qc.progress_apply(
        lambda x: get_count_cycle(x["well"], x["plate"], "01", dir_screen=dir_screen),
        axis=1,
    )
    tqdm.pandas(desc="Counting cycle 3")
    df_qc["count_cycle_3"] = df_qc.progress_apply(
        lambda x: get_count_cycle(x["well"], x["plate"], "03", dir_screen=dir_screen),
        axis=1,
    )
    df_qc["score"] = (df_qc["count_cycle_3"] + df_qc["count_cycle_1"]) / (
        2 * df_qc["count"]
    )
    df = (
        df.reset_index()
        .merge(df_qc, on=["well", "plate"], how="left")
        .drop(columns=["index"])
    )
    df = (
