...
from phenocoder.utils import setup_dask_slurm_client
        ...
        if use_dask:
            ...
            for well, plate in zip(df["well_id"], df["plate_id"]):
                future = client.submit(
                    get_spatial_stats, well, plate, mdata_copy, radii, radius_chull
                )
            ...
            results_deduplicated = [
                df.loc[:, ~df.columns.duplicated()] for df in results
            ]
