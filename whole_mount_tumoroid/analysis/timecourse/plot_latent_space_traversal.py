...
from whole_mount_tumoroid.phenocoder.phenocode import load_phenocoder

def get_patch(label, df_plates, patch_size=(100, 100), plot = False, calculate_regionprops=False):
    ...
    df_summary_plate = pd.read_csv(f'/pstore/data/ihb-tumoroid/data/processed/timecourse/{plate}/{plate}-01/features/nuclei/df_summary_TIF_OVR_BG.csv')
    ...
    if plot:
        ...
        axes[0].add_patch(plt.Rectangle((y0, x0), patch_size[1], patch_size[0],
...
def calc_latent_morphology_correlation(
    ...
    if adata_features is not None:
        feat_cols = morphology_cols if morphology_cols is not None else adata_features.var.index.tolist()
...
def plot_decoder_traversal_from_cell(
    ...
    patch, z, plate, patch_seg, props = get_patch(cell_label, df_plates, patch_size=patch_size, calculate_regionprops=False)
    ...
    traversal_values = np.linspace(center - n_std * std, center + n_std * std, n_samples)
    ...
    recon_img = model.decoder([z_anchor_tensor, cond_tensor_single]).numpy()  # (1, H, W, C)
    ...
    cond_tensor = tf.convert_to_tensor(np.tile(cond_encoded, (n_samples, 1)), dtype=tf.float32)
    imgs = model.decoder([z_tensor, cond_tensor]).numpy()

    # plot: original | reconstruction | traversal images
    fig, axes = plt.subplots(1, n_samples + 2, figsize=figsize,
    ...
    axes[0].set_xticks([])
    axes[0].set_yticks([])
    axes[0].text(patch.shape[1] / 2, 4, f'original',
                 color='white', fontsize=7, ha='center', va='top')
    axes[0].text(patch.shape[1] / 2, patch.shape[0] - 4, f'Z{dim_idx+1}={center:.2f}',
    ...
    axes[1].set_xticks([])
    axes[1].set_yticks([])
    axes[1].text(recon_img.shape[2] / 2, 4, 'reconstruction',
                 color='white', fontsize=7, ha='center', va='top')
    axes[1].text(recon_img.shape[2] / 2, recon_img.shape[1] - 4, f'{center:.2f}',
                 color='white', fontsize=7, ha='center', va='bottom')
    ...
    for i, ax in enumerate(axes[2:]):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.text(imgs.shape[2] / 2, imgs.shape[1] - 4, f'{traversal_values[i]:.2f}',
                color='white', fontsize=7, ha='center', va='bottom')
...
def orientation_embedding_correlation(
    ...
    sampled_labels = rng.choice(all_labels, size=min(n_sample, len(all_labels)), replace=False)
    ...
    for dim in tqdm(range(X.shape[1]), desc="Computing correlations"):
        ...
        results.append({
            "dim": dim,
            "r_sin": r_sin, "p_sin": p_sin,
            "r_cos": r_cos, "p_cos": p_cos,
...
def plot_orientation_correlation_results(results_df):
    top20 = results_df.nlargest(20, 'r_combined').sort_values('r_combined', ascending=False)
    
    heatmap_data = top20[['r_sin', 'r_cos', 'r_combined']].set_index(top20['dim'].astype(str))
...
mdata = mu.read_h5mu(
    Path(dir_results, "mdata_registered_imputed_mlp_normalized.h5mu")
)

df_plates = pd.concat([pd.read_csv(f'{dir_screen}/{plate}/plate_information.csv', dtype={plate:str}).assign(plate=plate) for plate in plates])

plates = params['plates']
df_plates = []
for plate in plates:
    df_plate = load_plate(plate, params['input_type'], params['dir_screen'], params['registered'], z_step=10, plate_id=f'{plate}-01')
...
df_corr, df_pval = calc_latent_morphology_correlation(
   adata, 
   df_plates, 
   morphology_cols=['area', 'eccentricity', 'major_axis_length', 'minor_axis_length', 
                    'ch_01_nuclei', 'ch_01_neighbors', f'neighbor_count_100'],
   n_components=8,
   use_pca=False,
)
...
fig = plot_latent_morphology_correlation(
    df_corr_imputed, df_pval_imputed, plot_only_top=20, figsize=(12, 8), sort_corr_order=True
)
...
labels = adata.obs_names[np.random.choice(len(adata.obs_names), 10, replace=False)].tolist()
for label in labels:
    dim_idx = 46
    patch, z, plate, patch_seg, props = get_patch(label, df_plates, patch_size = patch_size, plot = False, calculate_regionprops=True)
...
results_df, orientations = orientation_embedding_correlation(adata, df_plates, n_sample=1000, patch_size = patch_size)
...
df_merged = df_merged[['area', 'eccentricity', 'major_axis_length', 'minor_axis_length',
       'ch_01_nuclei', 'ch_01_neighbors', 'neighbor_count_100', 'r_sin', 'r_cos', 'r_combined']]
fig = plot_latent_morphology_correlation(
    df_merged, plot_only_top=20, figsize=(4, 8), sort_corr_order=True
)


sampled_labels = adata.obs_names[np.random.choice(len(adata.obs_names), 1000, replace=False)].tolist()
