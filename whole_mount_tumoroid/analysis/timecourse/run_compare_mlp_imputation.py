...
from whole_mount_tumoroid.analysis.timecourse.plot_mlp_imputation import plot_evaluation, plot_summary
from whole_mount_tumoroid.analysis.timecourse.run_mlp_training import (
    data_setup,
...
if __name__ == '__main__':
    ...
    for imp, w, mod in zip(imputations, weights, mods):
        ...
        for group in groups:
            ...
            baseline_df = evaluate_linear_baseline(
                lin_model, scaler,
                embeddings.numpy(), marker_ids.numpy(), intensities.numpy(),
                group, stains
            )
            baseline_group_metrics.append(baseline_df)

        # -- Load kNN results ---
        with open(f'/pstore/data/ihb-g-deco/USERS/schulzp9/git/tumoroid_screen/whole_mount_tumoroid/analysis/timecourse/cv_imputation/new_{mod}_{imp}.pkl', 'rb') as f:
            ...
            nn_imp_cv_melt = pd.melt(nn_imp_cv, id_vars=['marker'], var_name='imputation', value_name='Spearman').reset_index(drop=True)
    ...
    melt_stack = []
    for i in range(2):
        with open(f'/pstore/data/ihb-g-deco/USERS/schulzp9/git/tumoroid_screen/whole_mount_tumoroid/analysis/timecourse/cv_imputation/all_runs_df_10_{i}.pkl', 'rb') as f:
            nn_imputation = pickle.load(f)
    ...
    melt_stack = []
    for i in range(2):
        with open(f'/pstore/data/ihb-g-deco/USERS/schulzp9/git/tumoroid_screen/whole_mount_tumoroid/analysis/timecourse/cv_imputation/all_runs_neighbors_df_10_{i}.pkl', 'rb') as f:
            nn_imputation = pickle.load(f)
    ...
    plot_summary(mlp_metrics, baseline_metrics, nn_metrics, stains, colors, datasets, jitter_points = False, save_plot = False)
