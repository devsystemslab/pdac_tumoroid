...
from whole_mount_tumoroid.analysis.inhibitors.generate_example_images import (
    order_genes,
    plot_dotplot,
    plot_organoid,
    plot_umap,
)
from whole_mount_tumoroid.analysis.timecourse.correct_examples import add_scalebar
from whole_mount_tumoroid.image_processing.montage import load_well, rgb_overlay
...
mods_vertical = []
for c in cycles:
    mdata_orig = mu.read_h5mu(
        Path(dir_results, f"mdata_cycle-{c}.h5mu")
    )
    plate_horizontal = []
    for plate in plates:
        print(m)
...
mdata_projected = mu.read_h5mu(
     Path(dir_results, "mdata_plate4_projected.h5mu")
 )
