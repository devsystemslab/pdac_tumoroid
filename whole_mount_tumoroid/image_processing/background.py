...
def load_well(dir_plate:str, well:str, channel:str, input_type:str) -> np.ndarray:
    ...
    df_images = df_images[(df_images['well_id'] == well) &
                          (df_images['channel_id'] == channel)].sort_values(by=['z_stack_id'], ascending=True)
    imgs = [io.imread(Path(row['dir_images'], row['file'])) for index, row in df_images.iterrows()]
    imgs = np.asarray(imgs)
    return imgs


def correct_background(well:str, channel:str, z_slice:str, bg_value:int, dir_plate:str, input_type:str, dir_output:str) -> str:
    """
    Correct background
    :param well:
    :param channel:
    ...
    df_images = df_images[
        (df_images['well_id'] == well) & (df_images['channel_id'] == channel) & (df_images['z_stack_id'] == z_slice)]
...
def process_well_estimate_bg(well:str, dir_plate:str, input_type:str, output_type:str = 'background') -> str:
    """
...
def process_well_bleach_correction(well: str, channel: str,
                                   median_filter: bool, correction_mode: str,
                                   dir_plate: str, input_type: str,
                                   dir_output: str) -> None:
    """
    ...
    basic = BaSiC(get_darkfield=True, smoothness_flatfield=1, max_reweight_iterations=1000)
    basic.fit(imgs)
    ...
    [io.imsave(Path(dir_output, file), img, check_contrast=False) for file, img in zip(files, imgs_corr)]


def process_plate(dir_plate:str, input_type:str, config:dict, df_metadata_plate:pd.DataFrame) -> None:
    """
    Process plate.
    :param dir_plate:
    :param input_type:
    ...
    if config['mode'] == 'basicpy':
        ...
        if 'multiplicative' in config:
            if plate_id in config['multiplicative']:
                ...
                df_iter.loc[df_iter['channel_id'].isin(multiplicative_channels), 'mode'] = 'multiplicative'
        ...
        if 'median_filter' in config:
            if plate_id in config['median_filter']:
                ...
                df_iter.loc[df_iter['channel_id'].isin(median_filter_channels), 'median_filter'] = True

        # setup dask client
        client, cluster = setup_dask_client(memory='64GB', n_processes=4, n_cores=4, gpu=False,
                                            task_name=f'bleach_correction_{input_type}',
                                            log_directory=Path(dir_plate, 'logs'))
        # bleach correction
        process_well_bleach_correction_partial = partial(process_well_bleach_correction,
                                                         dir_plate=dir_plate,
                                                         input_type=input_type,
                                                         dir_output=dir_output)
        results_bleach_corr = []
        futures = client.map(process_well_bleach_correction_partial,
                             *[df_iter['well_id'].to_list(), df_iter['channel_id'].to_list(),
                               df_iter['median_filter'].tolist(),df_iter['mode'].tolist()], pure=False)
        ...
        client, cluster = setup_dask_client(memory='32GB', n_processes=4, n_cores=4, gpu=False,
        ...
        df_bg = []
        process_well_partial = partial(process_well_estimate_bg, dir_plate=dir_plate, input_type=input_type)
        futures = client.map(process_well_partial, wells, pure=False)
        for future, result in as_completed(futures, with_results=True):
            df_bg.append(result)
        df_bg = pd.concat([pd.read_csv(file, dtype={'channel': object, 'z_slice': object}) for file in df_bg])
        df_bg.to_csv(Path(dir_output_bg, f'df_bg_{input_type}.csv'), index=False)

        # background correction
        correct_background_partial = partial(correct_background, dir_plate=dir_plate,
                                             input_type=input_type, dir_output=dir_output)
        results_bg = []
        futures = client.map(correct_background_partial,
                             *[df_bg['well'].to_list(), df_bg['channel'].to_list(), df_bg['z_slice'].to_list(),
                               df_bg['bg_value'].to_list()], pure=False)
