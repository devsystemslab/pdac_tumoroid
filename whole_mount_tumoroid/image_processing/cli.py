import yaml
from whole_mount_tumoroid.image_processing import (preprocessing, mip, montage, flatfield, utils, segmentation,
                                                   features, neighborhoods, background, stitching3d, registration)
import pandas as pd
...
def default_config() -> dict:
    def_config = {
        ...
        'overlap': 0.1,
        'wells': None,
        'preprocessing': {'run':True,
                          'illuminaton_correction': True,
                          'remove_first_tile': True},
        'background_correction': {'run': True,
                              'mode':'basicpy', # 'basic' or 'smo'
                              'multiplicative':{'plate_id_x':['channel0d'], 'plate_id_y':['channel0d']},
        ...
        'feature_extraction': {'run':True, 'input': ['TIF_OVR','TIF_OVR_BG'], 'segmentation_input': 'SEG_TIF_OVR_BG'},
        'registration': {'run':True, 'input': ['TIF_OVR_BG'],
                         'source': '01',
                         'target': '03',
                         'z_step': 10,
                         'pixel_size': 0.322},
...
def run_pipeline(plate:str, config_file:str) -> None:
    ...
    df_metadata = df_metadata[(df_metadata['plate'] == plate) & (df_metadata['screen'] == config['screen'])]
    assert len(df_metadata) >= 1, f'Plate {plate} not found in metadata or for {config["screen"]}!'
    ...
    for plate_id in plate_ids:
        ...
        if Path(config['dir_output'], config['screen'], plate, 'plate_information.csv').is_file():
            df_plates_processed = pd.read_csv(Path(config['dir_output'], config['screen'],
                                                   plate, 'plate_information.csv'), dtype={'plate':str})
            df_processed = df_plates_processed[(df_plates_processed['plate_id'] == plate_id)]
            if len(df_processed) > 0:
                df_plates.append(df_processed)
                if config['overwrite_existing']:
                    print(f'Plate {plate_id} already processed. Overwriting existing data...')
                else:
        ...
        df_plate = utils.load_plate_information(screen=config['screen'],
        ...
        if config['preprocessing']['run']:
            ...
            for i,wells in enumerate(config['wells']):
                ...
                if config['preprocessing']['illuminaton_correction']:
                    ...
                    flatfield.run_basic(dir_images=df_plate['dir_raw'].iloc[0],
                ...
                preprocessing.process_plate(df_plate=df_plate,
                                            dir_plate=dir_plate,
                                            remove_first_tile=config['preprocessing']['remove_first_tile'],
                                            wells=wells)
        ...
        if config['background_correction']['run']:
            ...
            background.process_plate(dir_plate, input_type='TIF_OVR',
        ...
        if config['segmentation']['run']:
            for input_type in config['segmentation']['input']:
                ...
                segmentation.process_plate(dir_plate, input_type, config['segmentation']['restore_dapi'])
        ...
        if config['feature_extraction']['run']:
            for input_type in config['feature_extraction']['input']:
                ...
                features.process_plate(dir_plate, input_type, config['feature_extraction']['segmentation_input'])
                neighborhoods.process_plate(dir_plate, input_type, config['feature_extraction']['segmentation_input'])
    ...
    if len(df_plates) > 0:
        ...
        df_plates.to_csv(Path(config['dir_output'], config['screen'], plate, 'plate_information.csv'), index=False)

    # run registration
    if config['registration']['run']:
        for input_type in config['registration']['input']:
            print(f'Running registration for {input_type}...')
            registration.process_plate(Path(config['dir_output'], config['screen'], plate),
