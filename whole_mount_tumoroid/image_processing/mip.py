...
def process_well_mip(well, channel, dir_output_plate, dir_images):
    ...
    # process images
    processed_files = process_images(df_images=df_images,
...
def process_plate(dir_plate, input_type):
    ...
    :return:
    """
    client, cluster = setup_dask_client(memory='16GB', walltime='1:00', n_processes=2, n_cores=2, gpu=False,
                                        task_name=f'mip_{input_type}', log_directory=Path(dir_plate,'logs'))
    # get image metadata
