...
def stitch_montage(imgs, overlap, correction_factor=0.007):
    ...
    if n_images == 4:
        # stitch vertically
        imgs = np.asarray([stitch_images(imgs[::2], overlap=overlap, axis=0),
                           stitch_images(imgs[1::2], overlap=overlap, axis=0)])
        # stitch horizontally
        img = stitch_images(imgs, overlap=overlap, axis=1)
    elif n_images == 9:
        # stitch first row
        imgs_first_row = stitch_images(stitch_images(imgs[0], imgs[1], overlap=overlap, axis=1),
                                       imgs[2],
                                       overlap=overlap,
                                       axis=1)
        # stitch second row
        imgs_second_row = stitch_images(stitch_images(imgs[3], imgs[4], overlap=overlap, axis=1),
                                        imgs[5],
                                        overlap=overlap,
                                        axis=1)
        # stitch third row
        imgs_third_row = stitch_images(stitch_images(imgs[6],
                                                     imgs[7],
                                                     overlap=overlap, axis=1),
                                       imgs[8],
                                       overlap=overlap,
                                       axis=1)
        # stitch vertically
        ...
        imgs_third_row = stitch_images(stitch_images(imgs[6],
                                       ...
                                       axis=1)
        # stitch fourth row
        imgs_fourth_row = stitch_images(stitch_images(imgs[9],
                                                      imgs[10],
                                                      overlap=overlap, axis=1),
                                        imgs[11],
                                        overlap=overlap,
                                        axis=1)
        # stitch vertically
        imgs = np.asarray([imgs_first_row, imgs_second_row, imgs_third_row, imgs_fourth_row])
        img = stitch_images(imgs[0], imgs[1], overlap=overlap, axis=0)
        img = stitch_images(img, imgs[2], overlap=overlap, axis=0)
...
def process_images(well, z_plane, channel, dir_output_plate, img_flatfield, dir_images, df_images, overlap=None):

    df_images_select = df_images[(df_images['well_id'] == well) &
    ...
    if len(files) not in [4,9,16]:
        print(f'Found {len(files)} images for well {well}, z_plane {z_plane}, channel {channel}. Which is not 4, 9 or 16.')
        return []
    imgs = [correct_image(io.imread(Path(dir_images, file)), img_flatfield) for file in files]
...
def process_well_preprocessing(well, channel, dir_output, dir_output_plate, dir_images, overlap=None, correct=True, remove_first_tile=True):

    # load flatfield image
    if correct:
        img_flatfield = load_flatfield(channel,dir_flatfield=Path(dir_output, 'flatfield'))
    else:
        img_flatfield = None
    ...
    for z_plane in df_iter['z_stack_id'].tolist():
        processed_files.extend(process_images(well=well,
                               z_plane=z_plane,
                               channel=channel,
...
def process_plate(df_plate,
    ...
    :return:
    """
    client, cluster = setup_dask_client(walltime='6:00', n_processes=10, n_cores=10, gpu=False,
    ...
    process_well_partial = partial(process_well_preprocessing, overlap=overlap, dir_output=dir_plate,
                                   dir_output_plate=dir_output_plate, dir_images=dir_images,
                                   correct=True,
                                   remove_first_tile=remove_first_tile)
    futures = client.map(process_well_partial,
                         *[df_iter['well_id'].to_list(), df_iter['channel_id'].to_list()], pure=False)
