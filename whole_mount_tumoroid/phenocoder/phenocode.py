...
from whole_mount_tumoroid.phenocoder.utils import load_features
from whole_mount_tumoroid.phenocoder.generator import NucleiPatchGenerator, GridPatchGenerator
import yaml
import pandas as pd
...
def load_phenocoder(directory):
    ...
    if config['conditional']:
        model = CondCVAE(input_shape=tuple(config['input_shape']),
        ...
        model = CVAE(input_shape=tuple(config['input_shape']),
                     latent_dim=config['n_latent_dim'],
                     dense_dim=config['n_dense_dim'],
                     conv_layers=tuple(config['conv_layers']))
    model.compile()
...
def encode_nuclei_patches(well_ids, plate, dir_screen, cycle, dir_model, input_type='TIF_OVR_BG',
                          use_registered=False,
                          label_type=None,
                          df_labels=None,
                          return_only_patches=False,
                          filter_encodable_conditions=False,
                          message_passing=True, radius=100,
    ...
    nuclei_patch_generator = NucleiPatchGenerator(f'{dir_screen}/{plate}/{cycle}/{input_type}')
    results = []
    for well in well_ids:
        if use_registered:
            ...
            if label_type is not None:
             ...
             raise ValueError('Label type must be provided when using registered data')
        else:
            df = load_features(well, dir_plate=dir_plate, cycle=cycle, input_type=input_type).assign(well_id=well, plate_id=cycle)
            ...
            df = df[[col for col in df.columns if not re.match(general_pattern, col) or re.search(channel_pattern, col)]]
            if df.empty:
                continue
            df['z'] = df['z'] + 1
        ...
        patches, df = nuclei_patch_generator.get_patches(well, df, scale=True, quantiles_low=config['quantiles_low'], quantiles_high=config['quantiles_high'], channels=channels)
        df = df.reset_index()
        if df.empty:
            continue
        if return_only_patches:
            ...
            if config['conditional']:
                ...
                if filter_encodable_conditions:
                    ...
                    idx = df_cond.index[(df_cond['z'].isin(oh_enc.categories_[1])) &
                                        (df_cond['dataset'].isin(oh_enc.categories_[0]))]
                    conditions = oh_enc.transform(df_cond.iloc[idx][oh_enc.feature_names_in_])
                    patches = patches[idx]
                    df = df.iloc[idx]
                else:
            ...
            df.index = df['label'].astype(str) + '_' + df['well_id'] + '_' + df['plate_id']

            adata = ad.AnnData(X=df_z.values,
                               obs=df,
...
def merge_adata(adata_source, adata_target, make_labels_unique=False):
    ...
    if make_labels_unique:
        adata_source.obs['label'] = adata_source.obs['label'].astype(str) + '_' + adata_source.obs['well_id']
        adata_target.obs['label'] = adata_target.obs['label'].astype(str) + '_' + adata_target.obs['well_id']
    ...
    df = adata_target.obs.merge(
        adata_source.obs, on=['label','well_id','plate_id'], suffixes=('_target', '_source'))
    adata = ad.AnnData(X=np.concatenate([adata_target.X, adata_source.X], axis=1))
    adata.var_names = [name + '_target' for name in adata_target.var_names] + [name + '_source' for name in adata_source.var_names]
    adata.layers['message_passing'] = np.concatenate([adata_target.layers['message_passing'], adata_source.layers['message_passing']], axis=1)
    adata.obs = df

    return adata


def encode_grid_patches(well, plate, dir_screen, cycle, dir_model,
    ...
    cycle = f'{plate}-{cycle}'
    cvae, oh_enc, config = load_phenocoder(dir_model)
    grid_patch_generator = GridPatchGenerator(f'{dir_screen}/{plate}/{cycle}/{input_type}',
                                              stride=int(tuple(config['input_shape'])[0]/grid_resolution))
    patches, df = grid_patch_generator.get_patches(well,quantiles=config['quantiles_high'])
    ...
    if config['conditional']:
        ...
        if filter_encodable_conditions:
            ...
            idx = df_cond.index[(df_cond['z'].isin(oh_enc.categories_[1])) &
                                (df_cond['dataset'].isin(oh_enc.categories_[0]))]
            conditions = oh_enc.transform(df_cond.iloc[idx][oh_enc.feature_names_in_]).toarray()
            patches = patches[idx]
            df = df.iloc[idx]
        else:
            conditions = oh_enc.transform(df_cond[oh_enc.feature_names_in_]).toarray()
        z_mean, z_log_var, z = cvae.encoder.predict([patches, conditions], batch_size=64)
    else:
        z_mean, z_log_var, z = cvae.encoder.predict(patches, batch_size=64)
