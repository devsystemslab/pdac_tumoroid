...
from skimage.util import dtype_limits, montage
from tqdm import tqdm

from whole_mount_tumoroid.phenocoder.phenocode import encode_nuclei_patches
from whole_mount_tumoroid.phenocoder.utils import get_metadata, scale_image
...
def plot_organoid_with_image(
    ...
    if lut_dict is None:
        if cycle == "03":
            lut_dict = {"01": (1, 99), "02": (1, 99), "03": (95, 99), "04": (1, 99)}
        else:
            lut_dict = {"01": (1, 99), "02": (1, 99), "03": (1, 99), "04": (1, 99)}
    ...
    df_segmentation = get_metadata(
        Path(dir_screen, plate, f"{plate}-{cycle}", "SEG_TIF_OVR_BG")
    )
    ...
    for channel in df_images["channel_id"].unique():
        ...
        imgs_channel = np.asarray(
            [
                io.imread(
                    Path(dir_screen, plate, f"{plate}-{cycle}", "TIF_OVR_BG", file)
                )
                for file in tqdm(
                    df_images_channel["file"],
                    desc=f"Loading channel {channel}",
                    total=len(df_images_channel),
                )
            ]
        )
        ...
        range_lut = (
            int(np.percentile(imgs_channel, lut_dict[channel][0])),
            int(np.percentile(imgs_channel, lut_dict[channel][1])),
        )
        imgs_channel = rescale_intensity(
            imgs_channel, in_range=range_lut, out_range=(0, 1.0)
        )
    ...
    for channel in range(imgs.shape[0]):
        ...
        if save:
            ...
            io.imsave(
                Path(dir_output, f"{channel}-{suffix}.png"),
                rescale_intensity(imgs[channel], out_range=(0, 255)).astype(np.uint8),
            )
...
class RGBRotate(object):
    ...
    def apply_to_image(self, img):
        ...
        if img.shape[-1] == 3:
            img_rx = (
                img[..., 0] * self.matrix[0][0]
                + img[..., 1] * self.matrix[0][1]
                + img[..., 2] * self.matrix[0][2]
            )
            img_gx = (
                img[..., 0] * self.matrix[1][0]
                + img[..., 1] * self.matrix[1][1]
                + img[..., 2] * self.matrix[1][2]
            )
            img_bx = (
                img[..., 0] * self.matrix[2][0]
                + img[..., 1] * self.matrix[2][1]
                + img[..., 2] * self.matrix[2][2]
            )
...
def generate_examples(
    ...
    if n is not None:
        df_iter = (
            adata_org.obs.groupby("leiden", observed=False)
    ...
    processed_files = []
    for i, row in tqdm(
        df_iter.iterrows(), desc="Generating example images", total=df_iter.shape[0]
    ):
        for cycle in cycles:
            dir_images = os.path.join(
                dir_screen, row["plate_id"], row["plate_id"] + f"-{cycle}", input_type
            )
            files = [f for f in os.listdir(dir_images) if f.endswith(".tif")]
            files_well = sorted(
                [f for f in files if "_" + row["well_id"] + "_" in f], reverse=True
            )
            file_out = (
                row["well_id"]
                + "_"
            ...
            if len(files_well) == 0:
                ...
                for channel, file in enumerate(files_well):
                    ...
                    range_lut = (
                        np.percentile(img, lut_dict[cycle][channel][0]),
            ...
            if n_down_sampling is not None:
                imgs = np.asarray(
                    [img[::n_down_sampling, ::n_down_sampling] for img in imgs]
                )
            # move color channel to last axis
            imgs = np.moveaxis(imgs, 0, -1)
            img = imgs[..., 1:]
    ...
    if cycles == ("01", "03"):
        df_iter = df_iter.assign(
            file_cycle_1=[
                file for file in processed_files if file.endswith("cycle_01.png")
            ],
            file_cycle_3=[
                file for file in processed_files if file.endswith("cycle_03.png")
            ],
        )
    else:
        df_iter = df_iter.assign(
...
def plot_organoid_test(adata, res=1, layer=None):
    ...
    for i, well in enumerate(well_ids):
        ...
        ax.scatter(
            ...
            adata_well.obs["y"],
            adata_well.obs["z"],
            c=adata_well.obs["leiden"].astype(int),
            s=20,
            cmap=cmap,
            alpha=0.75,
        )
        ...
        sc.pl.scatter(
            adata_well, color="leiden", x="x", y="y", size=40, show=False, ax=ax
        )
