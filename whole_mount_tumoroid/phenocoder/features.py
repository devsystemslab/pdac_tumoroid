...
from whole_mount_tumoroid.phenocoder.utils import suffix_to_prefix, load_plate, average_matched_nuclei
from whole_mount_tumoroid.phenocoder.phenocode import encode_nuclei_patches, merge_adata
from whole_mount_tumoroid.phenocoder.qc import qc_empty_wells
...
def scale_per_plate(adata: ad.AnnData) -> ad.AnnData:
    ...
    adata = sc.concat(adatas=[scale(adata[adata.obs['plate'] == plate]) for plate in plates])
    return adata
...
def add_morphology(adata: ad.AnnData) -> ad.AnnData:
    ...
    adata_morph = ad.AnnData(
        np.concatenate([adata.X, adata.obs[['eccentricity', 'major_axis_length', 'minor_axis_length', 'area']].values],
                       axis=1))
    adata_morph.obs = adata.obs.drop(columns=['eccentricity', 'major_axis_length', 'minor_axis_length', 'area'])
...
def process_features(df: pd.DataFrame,
                     ...
                     ridge: bool = True,
                     registered: bool = True) -> ad.AnnData:
    """
    ...
    if registered:
        ...
        features = ['centroid-1', 'centroid-0', 'z', 'eccentricity', 'major_axis_length', 'minor_axis_length', 'area']
...
def run_feature_processing(plates: list,
                           markers: dict,
                           dir_screen: str,
                           input_type: str,
                           registered: bool,
    ...
    for plate in plates:
        if registered:
            ...
            df = load_plate(plate, input_type=input_type,
                            dir_screen=dir_screen,
                            registered=registered)

            df = qc_empty_wells(df, dir_screen=dir_screen,
                                threshold_score=qc_score_threshold,
                                threshold_distance=qc_distance_threshold)
            wells = df[df['plate'] == plate]['well'].unique().tolist()
        else:
            file = Path(dir_screen, plate, f'{plate}-{cycle}', 'features', 'nuclei', f'df_summary_{input_type}.csv')
        ...
        if registered:
            adata_source = encode_nuclei_patches(well_ids=wells,
                                                 plate=plate,
                                                 dir_screen=dir_screen,
            ...
            adata_target = encode_nuclei_patches(well_ids=wells,
                                                 plate=plate,
                                                 dir_screen=dir_screen,
            ...
            features_avg = ['centroid-1', 'centroid-0', 'z',
            ...
            adata = encode_nuclei_patches(well_ids=wells,
                                          plate=plate,
                                          dir_screen=dir_screen,
                                          cycle=cycle,
    ...
    adata_nuc = process_features(adata_pheno.obs, markers,
                                 feature_type='nuclei',
                                 morphology=True,
                                 scale_grouped=False,
                                 ridge=True,
    ...
    adata_msg = process_features(adata_pheno.obs, markers,
    ...
    mdata = mu.MuData({'nuclei': adata_nuc, 'nuclei_msg': adata_msg, 'phenocoder': adata_pheno})

    if 'phenocoder' in mdata.mod_names and 'phenocoder_msg' not in mdata.mod_names:
        if np.sum(mdata['phenocoder'].obs.columns.isin(['z', 'centroid-0', 'centroid-1'])) != 3:
            raise ValueError('phenocoder modality must have z, centroid-0, centroid-1 columns')
        mdata.mod['phenocoder_msg'] = mdata['phenocoder'].copy()
        mdata['phenocoder_msg'].X = mdata['phenocoder_msg'].layers['message_passing'].copy()
