...
def regular_grid_neighbors(sample, z, dir_plate, input_type, dir_out, dir_tmp):
    ...
    df_images = df_images[(df_images["well_id"] == sample) & (df_images["z_stack_id"] == z)]
    # sort by channel_id
    df_images = df_images.sort_values(by="channel_id")
    # run laminator for feature extraction
    la = LaminarNeighbors(radius=50)
    img_files = [Path(dir_images, file) for dir_images, file in zip(df_images["dir_images"], df_images["file"])]
    la.load_images(img_files)
    la.set_image_names([f"ch_{channel}" for channel in df_images["channel_id"].to_list()])
    # load open mask
    ...
    df_pivot = df_merged.pivot(index=["idx_x", "idx_y"], columns="image", values="intensity")
...
def message_passing(sample, z, dir_plate, input_type, segmentation_input, dir_out):
    ...
    if df_label.shape[0] == 0:
        return sample
    df_select = df_images[(df_images["well_id"] == sample) & (df_images["z_stack_id"] == z)]
    # sort by channel_id
    df_select = df_select.sort_values(by="channel_id")
    # load positions
    la = LaminarNeighbors(radius=50)
    la.load_label_image(path=Path(dir_plate, segmentation_input, df_label["file"].iloc[0]))
    ...
    if n_labels <= 2:
        ...
        # load images
        la.load_images([Path(dir_plate, input_type, file) for file in df_select["file"]])
        la.set_image_names([f"ch_{channel}" for channel in df_select["channel_id"].to_list()])
        # load open mask
