...
def register_pointclouds(
    ...
    if global_registration:
        ...
        registered = o3d.pipelines.registration.registration_icp(
            pcd_target, pcd_source, threshold, result_ransac.transformation
        )
    else:
        registered = o3d.pipelines.registration.registration_icp(
            pcd_target, pcd_source, threshold
        )
...
def link_points(
    ...
    if source.shape[0] < target.shape[0]:
        ...
        source_label, target_label = (
            source_label[np.squeeze(selected)],
            target_label[(indices[selected])],
        )
    else:
        ...
        target_label, source_label = (
            target_label[np.squeeze(selected)],
            source_label[(indices[selected])],
        )
...
def plot_registration_3d(source_init, target_init, source_final, target_final):
    ...
    ax.scatter(
        source_init[:, 0], source_init[:, 1], source_init[:, 2], s=0.5, alpha=0.5
    )
    ax.scatter(
        target_init[:, 0], target_init[:, 1], target_init[:, 2], s=0.5, alpha=0.5
    )
    ax.title.set_text("Before registration")
    ax = fig.add_subplot(1, 2, 2, projection="3d")
    ax.scatter(
        source_final[:, 0], source_final[:, 1], source_final[:, 2], s=0.5, alpha=0.5
    )
    ax.scatter(
        target_final[:, 0], target_final[:, 1], target_final[:, 2], s=0.5, alpha=0.5
    )
...
def process_well_register_nuclei(
    well: str,
    dir_plate: str,
    ...
    :return:
    """
    df_source = get_centroids(
        load_features(well, dir_plate, source_cycle, input_type), z_step, pixel_size
    )
    df_target = get_centroids(
        load_features(well, dir_plate, target_cycle, input_type), z_step, pixel_size
    )
    if df_source is None or df_target is None:
        ...
        source, target, source_label, target_label, distances = link_points(
            source, target, source_label, target_label, max_distance=100
        )
...
def process_plate(
    ...
    df_plates = pd.read_csv(
        Path(dir_plate, "plate_information.csv"), dtype={"plate": str}
    )
    dir_images_source = Path(
        df_plates[df_plates["plate_id"] == source_cycle]["dir_processed"].values[0],
        input_type,
    )
    dir_images_target = Path(
        df_plates[df_plates["plate_id"] == target_cycle]["dir_processed"].values[0],
        input_type,
    )
    ...
    processed_files = []
    df_iter = pd.merge(
        df_images_source.groupby("well_id").size().reset_index(),
        df_images_target.groupby("well_id").size().reset_index(),
        on="well_id",
        how="inner",
    )
    process_well_partial = partial(
        process_well_register_nuclei,
        dir_plate=dir_plate,
