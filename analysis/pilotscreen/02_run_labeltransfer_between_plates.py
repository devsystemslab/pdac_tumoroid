...
from phenocoder.cluster import (
    clustering,
    project_clustering,
    build_search_tree,
    majority_voting,
...
for cyc in cycle:
    ...
    for mod in mods:
        ...
        adata = clustering(
            ...
            dry_run=True,
            run_pca=True,
            n_comps=params["phenocoder"]["n_comps_pca"][mod],
            use_gpu=False,
        )
        plate4_subset = adata[keep_cyc]

        adata_sampled = plate4_subset[
            np.random.choice(
                plate4_subset.obs.index,
                int(frac * plate4_subset.shape[0]),
                replace=False,
            )
        ].copy()
        adata = project_clustering(
            adata,
            adata_sampled,
            ref_cluster_key="leiden",
            cluster_key_added="plate_4_projected",
        )
    ...
    for mod in mods:
        ...
        for col, (key, (orig, new)) in enumerate(subsets.items()):
            ...
            axes[col].set_xticklabels(
                axes[col].get_xticklabels(), rotation=0, ha="right"
            )
            axes[col].set_yticklabels(axes[col].get_yticklabels(), rotation=0)

        plt.suptitle(
            f"leiden vs plate_4_projected (row-normalized) | cycle {cyc} - {mod}",
            y=1.02,
        )
        plt.tight_layout()
