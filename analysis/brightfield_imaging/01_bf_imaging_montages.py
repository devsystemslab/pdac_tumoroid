...
from analysis.utils import annotate_img, scale_image
...
def generate_plate_montage(
    ...
    imgs = [scale_image(img) for img in tqdm(imgs, desc=f"Scaling images", total=len(imgs))]
    # annotate each image
    if annotate:
        imgs = [
            annotate_img(img, annotation=well, size=75)
            for img, well in tqdm(zip(imgs, wells), desc="Annotating images", total=len(imgs))
        ]
        # convert to grayscale
        imgs = [rgb2gray(img) for img in tqdm(imgs, desc="Converting to grayscale", total=len(imgs))]
    # downsample
...
def generate_timecourse_montage(well, dir_images, dir_output, down_sample_factor=None, shape=(2, 6)):
...
if __name__ == "__main__":
    ...
    for plate in plates:
        generate_plate_montage(plate, dir_images, dir_output, annotate=False, down_sample_factor=4)
