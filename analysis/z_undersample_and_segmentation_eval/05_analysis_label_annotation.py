...
Reports:
  ...
  * pixel / semantic:    foreground IoU, Dice, and mean IoU over matched instances
...
prediction. Because manual correction only *splits/adds* nuclei, FN dominates
and recall is the metric that moves.
"""
...
# matching
# --------------------------------------------------------------------------- #
def _match(gt_ids, pred_ids, iou, n_gt, n_pred, tau, max_hungarian=8000):
...
def _metrics_from_cont(cont, taus):
    ...
    rows = {}
    for tau in taus:
        tp, matched_iou = _match(gt_ids, pred_ids, iou, n_gt, n_pred, tau)
...
if __name__ == "__main__":
    ...
    dir_labels = (
       'data/segmentation_validation_inhibitors/labels'
    )
    samples = ["B08", "C08","D08"]
