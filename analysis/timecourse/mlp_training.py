...
def get_train_val_test_indices(
    n_samples, val_frac=0.15, test_frac=0.15, random_state=42
):
...
def dataloader_from_indices(
    embeddings, marker_ids, intensities, indices, batch_size=256, shuffle=True
):
...
class ConditionalMarkerMLP(nn.Module):
    ...
    def forward(self, dapi_emb: torch.Tensor, marker_ids: torch.Tensor) -> torch.Tensor:
        one_hot = torch.zeros(
            marker_ids.shape[0], self.num_markers, device=dapi_emb.device
        )
...
def train_mlp(
    ...
    for imputation in imputations:
        ...
        embeddings, marker_ids, intensities, obs_df = data_setup(
            adata, df_stain_layout, df_plates, imputation, stain_dict
        )
        ...
        train_loader = dataloader_from_indices(
            embeddings, marker_ids, intensities, train_idx, batch_size=256, shuffle=True
        )
        val_loader = dataloader_from_indices(
            embeddings, marker_ids, intensities, val_idx, batch_size=256, shuffle=False
        )
        ...
        torch.save(
            model.state_dict(),
            f"{mlp_dir}/{base_modality}_{imputation}_{timestamp}.pth",
        )
        print(
            f"MLP for {imputation} imputation saved to {mlp_dir}/{base_modality}_{imputation}_{timestamp}.pth"
        )
