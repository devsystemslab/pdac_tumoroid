...
dir_analysis <- 'whole_mount_tumoroid/analysis/timecourse'
...
read_history_files <- function(dataset, dir_data) {
  ...
  names(files) <- str_split(names(files), '_') %>%
    map_chr(function(x) { tail(x, 3) %>% str_c(sep = '_', collapse = '_') }) %>%
    str_remove_all("^[0-9-]+|[0-9]")
  df <- map(files, readr::read_csv) %>%
    bind_rows(.id = 'dataset') %>%
    separate(col = dataset, into = c('run','split', 'tag','type'), sep = '-') %>%
    mutate(type = str_replace(type, 'epoch_', '') %>% str_remove('.csv'),
           split = str_remove_all(split, '_')) %>%
    select(-tag)
...
df <- map(datasets, read_history_files, dir_data = dir_tensorboard) %>%
  bind_rows(.id = 'cycle') %>%
  mutate(cycle = str_replace(cycle, 'tumoroidscreen_cycle', 'cycle ') %>% str_to_sentence(),
...
p <- ggplot(df %>% filter(type != 'Learning rate'), aes(x = step, y = value, col = split)) +
  ...
  ylab(NULL) +
  xlab('Epoch')
ggsave(str_c(dir_analysis,'plots','train_history.pdf', sep='/'), p, dpi=72, width=90, height=45, units='mm')
