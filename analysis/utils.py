...
def add_chull_stats_to_mdata_org(
    ...
    adata.var_names = list(adata_init.var_names) + list(
        df_chulls_agg.columns.drop(["plate_id", "well_id"])
    )
...
def merge_org_embeddings(
    ...
    adata = ad.AnnData(
        X=np.concatenate(
            [adata_target.layers["raw"], adata_source.layers["raw"]], axis=1
        )
    )
    adata.obs = df
...
def run_chulls_connected_components(
    mdata: mu.MuData, clusters: list[str], mod: str = "phenocoder"
) -> pd.DataFrame:
    """
    ...
    df_iter = (
        mdata.mod[mod]
        .obs.groupby(["well_id", "plate_id"], observed=True)
        .size()
        .reset_index()
    )
    df_chulls = []
    for well, plate in tqdm(
        ...
        df_chulls_sample = get_chulls_connected_components(
            mdata.mod[mod], well, plate, clusters
        )
    ...
    agg_dict = {
        col: ["mean", "sum"]
        for col in df_chulls.columns
        if col not in ["well_id", "plate_id"]
    }
    df_chulls_agg = df_chulls.groupby(["well_id", "plate_id"]).agg(agg_dict)
    # reset row and column multi index
    df_chulls_agg.columns = [
        "_".join(col).strip() for col in df_chulls_agg.columns.values
    ]
    ...
    df_chulls_agg = df_iter.loc[:, ["well_id", "plate_id"]].merge(
        df_chulls_agg, on=["well_id", "plate_id"], how="left"
    )
